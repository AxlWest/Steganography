...
ImageFile* LSB_4_0_0::proformSteganography(ImageFile* image , const char *messagePathway)
{
    FILE* file = NULL ;
    unsigned int k = 0 ;
    ...
    for(int i = 0 ; i < fileSize ; i++) //Each loop hides 3 letters in 24 LSB's
    {
        ...
        {
            if(imageData[k] & 8) //Red byte LSB is also a 1
            {
                //Already a 1 do nothing
            }
            else //Red byte LSB is a 0
...
void LSB_4_0_0::extractSteganography(ImageFile* image , const char *messageFileName)
{
    FILE* file = NULL ;
    unsigned char* textData = NULL ;
    unsigned char* imageData = NULL ;
...
void LSB_4_0_0::addEnding(ImageFile *image , int k)
{
    //=<END!>=
